...
class NPProcuduralVC: UIViewController ,UITableViewDelegate,UITableViewDataSource {
    var arrProcedural:NSArray!
    @IBOutlet var lblIntra: UILabel!
    @IBOutlet var lblPost: UILabel!
    @IBOutlet var tblProcedural: UITableView!
    ...
    override func viewDidLoad() {
        ...
        arrProcedural                   = ["Vascular access complication","Pneumothorax"]
        
        tblProcedural.register(UINib(nibName: "QIMultiCell", bundle: nil), forCellReuseIdentifier: "QIMultiCell")
        tblProcedural.tableFooterView   = UIView()
        tblProcedural.allowsSelection   = false
        tblProcedural.estimatedRowHeight = UITableViewAutomaticDimension
    ...
    @IBAction func btnEnableTapped(_ sender: UIButton) {
        let buttonPosition      = sender.convert(CGPoint.zero, to: self.tblProcedural)
        let indexPath           = self.tblProcedural.indexPathForRow(at: buttonPosition)
        if indexPath != nil {
            
            
            let cell = tblProcedural.cellForRow(at: indexPath!) as! QIMultiCell
            if cell.btnSelect == sender
            {
                
                if dictSelected.object(forKey: arrProcedural.object(at: (indexPath?.row)!) as! String) as? String == "Intra Op"
                {
                    dictSelected.setValue(" ", forKey: arrProcedural.object(at: (indexPath?.row)!) as! String)
                    cell.btnSelect.isSelected = false
                    cell.btnUnSelect.isSelected = false
                    arrSelected.remove(arrProcedural.object(at: (indexPath?.row)!) as! String)
                }else{
                    dictSelected.setValue("Intra Op", forKey: arrProcedural.object(at: (indexPath?.row)!) as! String)
                    arrSelected.add(arrProcedural.object(at: (indexPath?.row)!) as! String)
                    cell.btnSelect.isSelected = true
                    cell.btnUnSelect.isSelected = false
                }
            }else{
                
                if dictSelected.object(forKey: arrProcedural.object(at: (indexPath?.row)!) as! String) as? String == "Post Op"
                {
                    dictSelected.setValue(" ", forKey: arrProcedural.object(at: (indexPath?.row)!) as! String)
                    arrSelected.remove(arrProcedural.object(at: (indexPath?.row)!) as! String)
                    cell.btnSelect.isSelected = false
                    ...
                    dictSelected.setValue("Post Op", forKey: arrProcedural.object(at: (indexPath?.row)!) as! String)
                    arrSelected.add(arrProcedural.object(at: (indexPath?.row)!) as! String)
                    cell.btnSelect.isSelected = false
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tblProcedural.dequeueReusableCell(withIdentifier: "QIMultiCell") as? QIMultiCell
        
        if cell == nil {
            cell = QIMultiCell(style: UITableViewCellStyle.value1, reuseIdentifier: "QIMultiCell")
        }
        cell?.lblTitle.text              = (arrProcedural.object(at: indexPath.row) as! NSString) as String
