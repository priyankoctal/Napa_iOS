...
class NPCardiaoQIVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
    var arrCardiov:NSArray!
    ...
    override func viewDidLoad() {
        ...
        arrCardiov                   = ["Myocardial ischemia requiring intervention","Myocardial infarction","Dysrhythmia rquiring treatment","Cardiac arrest(with CPR)"]
    ...
    @IBAction func btnBackTapped(_ sender: UIButton) {
        ...
        let yesAction = UIAlertAction(title: "Yes", style: .default) { (action) -> Void in
            ...
            self.QIServiceCardio(dict: dictQINew)
    ...
    func QIServiceCardio(dict:NSDictionary)
    {
        obj.ShowHud()
    ...
    @IBAction func btnEnableTapped(_ sender: UIButton) {
        ...
        if indexPath != nil {
                ...
                if cell.btnSelect == sender
                {
                    ...
                    if dictSelected.object(forKey: arrCardiov.object(at: (indexPath?.row)!) as! String) as? String == "Intra Op"
                    {
                        dictSelected.setValue(" ", forKey: arrCardiov.object(at: (indexPath?.row)!) as! String)
                        cell.btnSelect.isSelected = false
                        cell.btnUnSelect.isSelected = false
                        arrSelected.remove(arrCardiov.object(at: (indexPath?.row)!) as! String)
                    }else{
                        dictSelected.setValue("Intra Op", forKey: arrCardiov.object(at: (indexPath?.row)!) as! String)
                        arrSelected.add(arrCardiov.object(at: (indexPath?.row)!) as! String)
                        cell.btnSelect.isSelected = true
                        cell.btnUnSelect.isSelected = false
                    }
                }else{
                    
                    if dictSelected.object(forKey: arrCardiov.object(at: (indexPath?.row)!) as! String) as? String == "Post Op"
                    {
                        dictSelected.setValue(" ", forKey: arrCardiov.object(at: (indexPath?.row)!) as! String)
                        arrSelected.remove(arrCardiov.object(at: (indexPath?.row)!) as! String)
                        cell.btnSelect.isSelected = false
                        ...
                        dictSelected.setValue("Post Op", forKey: arrCardiov.object(at: (indexPath?.row)!) as! String)
                        arrSelected.add(arrCardiov.object(at: (indexPath?.row)!) as! String)
                        cell.btnSelect.isSelected = false
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            cell?.lblTitle.text              = (arrCardiov.object(at: indexPath.row) as! NSString) as String
        if indexPath.row==3 {
