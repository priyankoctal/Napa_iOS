...
class NPComplianceVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
    var arrRegional:NSArray!
    ...
    override func viewDidLoad() {
        ...
        arrRegional                   = ["Wet tap","Post-dural puncture H/A","Failed regional block as primary anesthetic","Local anesthetic toxicity","Excessive block requiring airway support","Epidural hematoma following regional","Peripheral nerve injury:in distriburion of regional anesthesia","Infection after a peripheral nerve block"]
    ...
    @IBAction func btnEnableTapped(_ sender: UIButton) {
        ...
        if indexPath != nil {
            ...
            if cell.btnSelect == sender
            {
                ...
                if dictSelected.object(forKey: arrRegional.object(at: (indexPath?.row)!) as! String) as? String == "Intra Op"
                {
                    dictSelected.setValue(" ", forKey: arrRegional.object(at: (indexPath?.row)!) as! String)
                    cell.btnSelect.isSelected = false
                    cell.btnUnSelect.isSelected = false
                    arrSelected.remove(arrRegional.object(at: (indexPath?.row)!) as! String)
                }else{
                    dictSelected.setValue("Intra Op", forKey: arrRegional.object(at: (indexPath?.row)!) as! String)
                    arrSelected.add(arrRegional.object(at: (indexPath?.row)!) as! String)
                    cell.btnSelect.isSelected = true
                    cell.btnUnSelect.isSelected = false
                }
            }else{
                
                if dictSelected.object(forKey: arrRegional.object(at: (indexPath?.row)!) as! String) as? String == "Post Op"
                {
                    dictSelected.setValue(" ", forKey: arrRegional.object(at: (indexPath?.row)!) as! String)
                    arrSelected.remove(arrRegional.object(at: (indexPath?.row)!) as! String)
                    cell.btnSelect.isSelected = false
                    ...
                    dictSelected.setValue("Post Op", forKey: arrRegional.object(at: (indexPath?.row)!) as! String)
                    arrSelected.add(arrRegional.object(at: (indexPath?.row)!) as! String)
                    cell.btnSelect.isSelected = false
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.lblTitle.text              = (arrRegional.object(at: indexPath.row) as! NSString) as String
        if indexPath.row==0 {
        ...
        if arrRegional.count-1 == indexPath.row
        {
