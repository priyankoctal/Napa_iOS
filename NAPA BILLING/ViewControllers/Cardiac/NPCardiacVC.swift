...
class NPCardiacVC: UIViewController ,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        arrPostOp   = ["Prep area","In OR prior to induction","In OR after induction","Off pump","Hypothermic circulatory arrest","Re-do CABG(>1month)","Pump procedure(<1 year of age)","TEE probe insertion only","TEE w/ interpretation and report","TEE for congenital anomalies","Doppler echo pulse wave","Doppler echo color flow","Chest and Abdomen"];
        setCardiacView()
    ...
    @IBAction func btnBackTapped(_ sender: UIButton) {
        ...
        let yesAction = UIAlertAction(title: "Yes", style: .default) { (action) -> Void in
            ...
            for i in 0 ..< self.arrSelected.count
            {
                ...
                dictCardiac.setValue("yes", forKey: "\(self.arrPostOp.object(at: self.arrSelected.object(at: i) as! Int) as! String)")
            }
            
            dictCardiac.setValue(self.txtPreStartTime.text, forKey: "Start Time")
            dictCardiac.setValue(self.txtPreEndTime.text, forKey: "End Time")
            dictCardiac.setValue("", forKey: "cardiac_and_tee_provider")
            ...
            self.SaveCardiacService(dict:dict)
    ...
    //SaveCardiacService webservice
    func SaveCardiacService(dict:NSDictionary)
    {
        obj.ShowHud()
